...
display(data.head(3))
display(
    "There are {} observations with {} features".format(
...
plot_bar("job"), plot_bar("marital"), plot_bar("education"), plot_bar(
    "contact"
...
data.housing = data.housing.map({"yes": 1, "unknown": 0, "no": 0}).astype(
    "uint8"
)
data.default = data.default.map({"no": 1, "unknown": 0, "yes": 0}).astype(
    "uint8"
)
data.pdays = data.pdays.replace(999, 0)  # replace with 0 if not contact
data.previous = data.previous.apply(lambda x: 1 if x > 0 else 0).astype(
    "uint8"
)  # binary has contact or not

# binary if were was an outcome of marketing campane
data.poutcome = data.poutcome.map(
    {"nonexistent": 0, "failure": 0, "success": 1}
).astype("uint8")

# change the range of Var Rate
data["emp.var.rate"] = data["emp.var.rate"].apply(
    lambda x: x * -0.0001 if x > 0 else x * 1
)
data["emp.var.rate"] = data["emp.var.rate"] * -1
data["emp.var.rate"] = (
    data["emp.var.rate"]
    .apply(lambda x: -np.log(x) if x < 1 else np.log(x))
    .astype("uint8")
)
...
def duration(data):
    ...
    data.loc[
        (data["duration"] > 102) & (data["duration"] <= 180), "duration"
    ] = 2
    data.loc[
        (data["duration"] > 180) & (data["duration"] <= 319), "duration"
    ] = 3
    data.loc[
        (data["duration"] > 319) & (data["duration"] <= 645), "duration"
    ] = 4
...
X_train, X_test, y_train, y_test = train_test_split(
    numeric_dataset, y, test_size=0.2, random_state=random_state
)
# collect excess data
gc.collect()
...
pipe_lr = Pipeline(
    [
        (
            "lr",
            LogisticRegression(
                random_state=random_state, n_jobs=n_jobs, max_iter=500
            ),
        )
    ]
)
...
pipe_dt = Pipeline(
    [
        (
            "dt",
            DecisionTreeClassifier(
                random_state=random_state, max_features="auto"
            ),
        )
    ]
)
        ...
        (
            ...
            BaggingClassifier(
                base_estimator=SGDClassifier(
                    random_state=random_state, n_jobs=n_jobs, max_iter=1500
                ),
...
pipe_nn = Pipeline(
    [
        (
            "NN",
            KerasClassifier(
                build_fn=create_nn, epochs=2, batch_size=16, activation="relu"
            ),
        )
    ]
)
...
pipeline = Pipeline([("NN", KerasClassifier(build_fn=create_nn))])
...
grid_params_lr = [
    {"lr__penalty": ["l2"], "lr__C": [0.3, 0.6, 0.7], "lr__solver": ["sag"]}
]
...
grid_params_dt = [
    {"dt__max_depth": [8, 10], "dt__min_samples_leaf": [1, 3, 5, 7]}
]
...
gs_lr = GridSearchCV(
    pipe_lr, param_grid=grid_params_lr, scoring="accuracy", cv=cv
)
# for RandomForestClassifier
gs_rf = GridSearchCV(
    pipe_rf, param_grid=grid_params_rf, scoring="accuracy", cv=cv
)
# for KNeighborsClassifier
gs_knn = GridSearchCV(
    pipe_knn, param_grid=grid_params_knn, scoring="accuracy", cv=cv
)
# for DecisionTreeClassifier
gs_dt = GridSearchCV(
    pipe_dt, param_grid=grid_params_dt, scoring="accuracy", cv=cv
)
# for BaggingClassifier
gs_bag = GridSearchCV(
    pipe_bag, param_grid=grid_params_bag, scoring="accuracy", cv=cv
)
# for SGDClassifier
gs_sgd = GridSearchCV(
    pipe_sgd, param_grid=grid_params_sgd, scoring="accuracy", cv=cv
)
# for NeuralNet
gs_nn = GridSearchCV(
    pipe_nn, param_grid=grid_params_nn, scoring="accuracy", cv=cv
)
...
for index, model in enumerate(look_for):
    ...
    print("---------------------------------------------")
    print(
        "ROC_AUC is {} and accuracy rate is {}".format(
...
pd.DataFrame(
    list(zip(model_dict.values(), result_acc.values(), result_auc.values())),
...
def graph(model, X_test, y_test):
    ...
    display(
        "max oob {} and number of estimators {}".format(
            max(obb), est[np.argmax(obb)]
        )
    )
    plt.plot(est, obb)
    plt.title("model")
...
def graph(model, X_train, y_train, X_val, y_val):
    ...
    for i in tqdm(est):
        keras_model = KerasClassifier(
            build_fn=create_nn, epochs=i, batch_size=10, verbose=0
        )
        keras_model.fit(X_train, y_train, validation_data=(X_val, y_val))
        y_pred = keras_model.predict(X_val)
        acc.append(accuracy_score(y_val, y_pred))
    display(
        "max accuracy {} and number of epochs {}".format(
            max(acc), est[np.argmax(acc)]
        )
    )
    plt.plot(est, acc)
    plt.title("model")
...
trace0 = go.Scatter(
    x=fpr, y=tpr, text=threshold, fill="tozeroy", name="ROC Curve"
)
...
def build_feature_importance(model, X_train, y_train):
    ...
    data = pd.DataFrame(
        models.feature_importances_, X_train.columns, columns=["feature"]
    )
