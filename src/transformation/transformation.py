    ...
    def get_data_transformer_object(self):
        ...
        try:
            ...
            def var_rate_transformer(X):
                emp_var_rate = X
                emp_var_rate = np.where(
                    emp_var_rate > 0, emp_var_rate * -0.0001, emp_var_rate * 1
                )
            ...
            binary_pipeline = Pipeline(
                [
                    ...
                    (
                        "pdays_binary",
                        FunctionTransformer(
                            lambda x: x.replace(999, 0).astype("uint8")
                        ),
                    ),
            ...
            signchange_pipeline = Pipeline(
                [
                    (
                        "confi_idx",
                        FunctionTransformer(
                            lambda x: x.apply(lambda x: -1 * x).astype("uint8")
                        ),
                    )
                ]
            )
            ...
            target_encoder = Pipeline(
                [
                    (
                        "target_encoding",
                        FunctionTransformer(target_encoding_wrapper),
                    )
                ]
            )
            ...
            preprocessor = ColumnTransformer(
                transformers=[
                    ...
                    (
                        "column_keep",
                        numeric_col_pipeline,
                        ["cons.price.idx", "age", "euribor3m", "campaign"],
                    ),
